...
$(function()
{
	...
	if (contentSlide.length > 1)
	{
		...
		contentSlider.delegate('div.slideSwitch div', 'click', function(event)
		{
			...
			activeSlide.find('div.information').animate({ marginLeft: '-420px', opacity: 0.1 }, 500, function()
			{
				activeSlide.hide();
				$(this).css({ marginLeft: 0, opacity: 1 });
			});
			
			activeSlide.find('img').animate({ marginRight: '-480px', opacity: 0.1 }, 500, function()
			{
				$(this).css({ marginRight: 0, opacity: 1.0 });
			});
			
			$('#slide-' + id).css({ position: 'absolute', top: '-340px' }).show().animate({ top: '0' }, 500, function()
			{
